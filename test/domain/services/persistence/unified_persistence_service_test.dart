...
///
/// Tests pour le service de persistance unifié qui remplace 36 services dupliqués
/// Vérifie le respect des principes SOLID et la fonctionnalité complète
...
void main() {
  group('UnifiedPersistenceService Tests', () {
    ...
    setUp(() {
      ...
      service = UnifiedPersistenceService(
        ...
        configuration: const UnifiedPersistenceConfiguration(
          ...
          enableBackgroundSync: false, // Désactivé pour les tests
        ),
      );
    });

    group('Lifecycle Management', () {
      test('initialise correctement en mode local pour utilisateur invité', () async {
        // Act
      ...
      test('initialise correctement en mode cloud pour utilisateur connecté', () async {
        // Act
      ...
      test('lance une exception si déjà initialisé', () async {
        // Arrange
        await service.initialize(isAuthenticated: false);
        ...
        expect(
          ...
          throwsA(isA<UnifiedPersistenceException>().having(
            (e) => e.message,
            'message',
            contains('déjà initialisé'),
          )),
        );
      });

      test('lance une exception si utilisé sans initialisation', () async {
        // Act & Assert
        expect(
          () => service.getAllLists(),
          throwsA(isA<UnifiedPersistenceException>().having(
    ...
    group('Authentication State Changes', () {
      ...
      test('gère la transition Invité → Connecté', () async {
        // Arrange
      ...
      test('gère la transition Connecté → Invité', () async {
        // Arrange
    ...
    group('List Operations - Local Mode', () {
      ...
      test('récupère toutes les listes en mode local', () async {
        // Arrange
        final testLists = [
      ...
      test('sauvegarde une liste en mode local', () async {
        ...
        verify(mockLogger.info(argThat(contains('sauvegardée')), context: anyNamed('context')));
      });

      test('gère les conflits de déduplication lors de la sauvegarde', () async {
        // Arrange
        final testList = TestDataFactory.createTestList();
      ...
      test('met à jour une liste en mode local', () async {
        // Arrange
        final testList = TestDataFactory.createTestList();
    ...
    group('Item Operations - Local Mode', () {
      ...
      test('gère les conflits de déduplication pour items', () async {
        // Arrange
        final testItem = TestDataFactory.createTestItem();
    ...
    group('Cloud Mode Operations', () {
      ...
      test('récupère les listes en mode cloud avec fallback', () async {
        // Arrange
        final cloudLists = [TestDataFactory.createTestList()];
        when(mockCloudRepository.getAllLists()).thenAnswer((_) async => cloudLists);
      ...
      test('sauvegarde en local puis cloud en mode cloudFirst', () async {
        ...
        // Assert
        verify(mockLocalRepository.saveList(testList)).called(1);
        // Note: Cloud sync est asynchrone et ne sera pas vérifié dans les tests
      });
    });
    ...
    group('Bulk Operations', () {
      ...
      test('sauvegarde plusieurs items avec succès', () async {
        // Arrange
        final items = [
          TestDataFactory.createTestItem(id: '1', title: 'Item 1'),
          TestDataFactory.createTestItem(id: '2', title: 'Item 2'),
      ...
      test('effectue un rollback en cas d\'échec partiel lors de bulk save', () async {
        // Arrange
        final items = [
          TestDataFactory.createTestItem(id: '1', title: 'Item 1'),
          TestDataFactory.createTestItem(id: '2', title: 'Item 2'),
        ...
        when(mockLocalItemRepository.add(items[1])).thenThrow(Exception('Échec sauvegarde'));
    ...
    group('Validation', () {
      ...
      test('vérifie la persistance d\'une liste avec succès', () async {
        // Arrange
        const listId = 'test-list-1';
        ...
        verify(mockLogger.debug(argThat(contains('Vérification réussie')), context: anyNamed('context')));
      });

      test('lance une exception si liste non trouvée après sauvegarde', () async {
        // Arrange
        const listId = 'test-list-1';
        ...
        expect(
          ...
          throwsA(isA<UnifiedPersistenceException>().having(
            (e) => e.message,
            'message',
            contains('non trouvée après sauvegarde'),
          )),
        );
      });

      test('vérifie la persistance d\'un item avec succès', () async {
        // Arrange
        const itemId = 'test-item-1';
    ...
    group('Statistics and Monitoring', () {
      ...
      test('retourne des statistiques complètes', () {
        // Act
      ...
      test('indique l\'état de synchronisation', () {
        // Assert
  ...
  group('UnifiedPersistenceServiceFactory Tests', () {
    ...
    test('crée un service avec configuration par défaut', () {
      // Act
      final service = factory.createDefault(logger: mockLogger);

      // Assert
      expect(service, isA<IUnifiedPersistenceService>());
      verify(mockLogger.info(argThat(contains('Création')), context: anyNamed('context')));
    });

    test('crée un service en mode local uniquement', () {
      // Act
    ...
    test('crée un service en mode cloud prioritaire', () {
      // Act
    ...
    test('crée un service en mode hybride', () {
      // Act
  ...
  group('Configuration Tests', () {
    ...
    test('UnifiedPersistenceConfiguration peut être personnalisée', () {
      // Act
      const config = UnifiedPersistenceConfiguration(
